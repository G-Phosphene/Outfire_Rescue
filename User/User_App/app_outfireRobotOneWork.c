...
{ 
	...
	if(outfireRobotState.workMode == OUT_FIRE){
			app_judgeFunc();	                                  	//	判断火点位数以及确定返程火点
		if(outfireRobotState.returnFlag[RETURN] == 0){					//  如果有火点，则返回标志位为0，即向前寻找火点
			outfireRobotState.robotTaskstep = FIRST_FIRE; 				//	第一火点
		...
		}else if(outfireRobotState.returnFlag[RETURN] ==	1){ 	//如果没火点，则返回标志位为1，即原地不动
			outfireRobotState.robotTaskstep = RETURN;							//原地不动
			outfireRobotState.step = DOING;
			outfireRobotState.workStep = NULL;
			outfireRobotState.moveWays = STOP;
			outfireRobotState.beginFlag = FREE;
...
void app_findFirstFire(void){
	if(outfireRobotState.fireArray[FIRST_FIRE] == 1){   									//第一火点有火，则执行以下动作
		switch(outfireRobotState.workStep){								
			case 0:{
				calibrationFinish = app_calibration(RIGHT);											//起步校准
			...
			case 1:{
				calibrationGostraight(RIGHT);																		//右侧前进校准函数
			...
			case 3:{
				calibrationFinish = app_findFire();															//寻火
			...
			case 6:{
				app_goOtherAction(150,NO_TURN,BEHIND);													//倒车
...
void app_returnFunc(void){																				//节点返程函数
	if(outfireRobotState.returnFlag[FIRST_FIRE] == 1){
		switch(outfireRobotState.workStep){
			case 0:{
				outfireRobotState.moveWays = REVERSE_LEFT_TURN_90;
